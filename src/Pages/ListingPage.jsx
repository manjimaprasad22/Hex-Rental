...
const ListingPage = () => {
  ...
  const [fromTime, setFromTime] = useState(location.state?location.state.fromTime:"00:00:00");
  const [toTime, settoTime] = useState(location.state?location.state.toTime:"00:00:00"); 
  const [selectcity, setselectcity] = useState(location.state?location.state.selectcity:1);
  const [selectlocation, setselectlocation] = useState(location.state?location.state.selectlocation:2);
  const [sortByLowToHigh, setSortByLowToHigh] = useState(false);
  const [sortByHighToLow, setSortByHighToLow] = useState(false);
  const [fromDate,setFromDate]=useState(location.state?location.state.fromDate:new Date())
  const [toDate,setToDate]=useState(location.state?location.state.toDate:tomorrow)
...
const handleLocation=(event)=>{
  ...
  console.log(event.target.value,'loc');
     ...
     const handleFromDateChange = (date) => {
      ...
      setFromDate(date);// reset to date when from date changes
    ...
    const getData=async()=>{
      ...
      await axios({
          ...
          setfacilities(response.data.Models.specifications)
          setloading(false)
          response.data.Models.forEach((el)=>{
  ...
  const handleBooking=async(id,offer,perday,currency)=>{
    if(token){
        ...
        <div className="container">
          <div className="row">
            ...
            <div className="col-lg-9  col-md-9 col-sm-12 Listing_block">
              ...
              {loading?(<><Skeleton style={{width:"100%"}} count={8}/>
               <br/><Skeleton style={{width:"100%"}} count={8}/>
               <br/><Skeleton style={{width:"100%"}} count={8}/></>):
              <div className="listing_flex">
              {sortedProducts.map((car)=>{
            return(
                <div className="single_block" key={car.Model_Id}>
                ...
                <ul>
                 { car.specifications.length>0?
                  car.specifications.map((spef, index)=>{
                      return(

                        <li key={index}>
                      <img style={{width:20,height:20}} src={REACT_APP_IMAGE_URL+spef[1]}/>&nbsp;
                      <span >{spef[0]}</span>&nbsp;
                ...
                <div>
                  ...
                  <div>
                    ...
                    {car.Model_Available>0?<button onClick={()=>{handleBooking(car.Model_Id,car.Offer_Rate,car.Rate_per_day,car.Currency_code)}}>Pay Now</button>:<button className='car-price-btn-out'>Rented Out</button>}
                  </div>
                </div>
              </div>
            </div>
